import { runAiFilter } from "./aiFilter.js";
...
function buildDecision(params) {
    const sizeMultiplier = params.score >= 92 ? 1.0 :
        params.score >= 86 ? 0.9 :
...
function evaluateSniper(m, ageSeconds, secondsSinceTrade, volToLiquidity, notDead) {
    ...
    const redFlags = [];
    if (!isNewEnough)
        redFlags.push(`zu alt (${ageSeconds}s)`);
    if (!liquidityEnough)
        redFlags.push(`zu wenig Liquidität ($${m.liquidityUsd.toFixed(1)})`);
    if (!volumeEnough)
        redFlags.push(`zu wenig 5m-Volumen ($${m.volume5mUsd.toFixed(1)})`);
    if (!avgVolumeEnough)
        redFlags.push(`zu wenig avg5m ($${m.avgVolume5mUsd.toFixed(1)})`);
    if (!volumeToLiquidityHealthy)
        redFlags.push(`vol/liq zu schwach (${volToLiquidity.toFixed(2)})`);
    if (!sampleCountEnough)
        redFlags.push(`zu wenige Samples (${m.sampleCount})`);
    if (!freshTrade)
        redFlags.push(`letzter Trade zu alt (${secondsSinceTrade}s)`);
    if (!momentumGood)
        redFlags.push(`5m-Momentum ungeeignet (${(m.change5mPct * 100).toFixed(2)}%)`);
    if (!reboundHealthy)
        redFlags.push(`Rebound ungeeignet (${(m.reboundFrom5mLowPct * 100).toFixed(2)}%)`);
    if (!notDumped)
        redFlags.push(`zu weit unter 1h-High (${(m.dropFrom1hHighPct * 100).toFixed(2)}%)`);
    if (!notDead)
        redFlags.push("Coin wirkt tot");
    const reasons = [];
    if (liquidityEnough)
        reasons.push(`Liquidität ok $${m.liquidityUsd.toFixed(1)}`);
    if (volumeEnough)
        reasons.push(`5m-Vol ok $${m.volume5mUsd.toFixed(1)}`);
    if (volumeToLiquidityHealthy)
        reasons.push(`vol/liq ${volToLiquidity.toFixed(2)}`);
    if (momentumGood)
        reasons.push(`5m ${(m.change5mPct * 100).toFixed(2)}%`);
    if (reboundHealthy)
        reasons.push(`Rebound ${(m.reboundFrom5mLowPct * 100).toFixed(2)}%`);
    if (freshTrade)
        reasons.push(`frischer Flow ${secondsSinceTrade}s`);
    const buy = isNewEnough &&
        liquidityEnough &&
        volumeEnough &&
        avgVolumeEnough &&
...
function evaluateTrend(m, ageSeconds, secondsSinceTrade, volToLiquidity, notDead) {
    ...
    const redFlags = [];
    if (!oldEnough)
        redFlags.push(`noch zu frisch für Trend (${ageSeconds}s)`);
    if (!ageAllowed)
        redFlags.push(`Trend zu alt (${ageSeconds}s)`);
    if (!liquidityEnough)
        redFlags.push(`Trend-Liq zu klein ($${m.liquidityUsd.toFixed(1)})`);
    if (!volumeEnough)
        redFlags.push(`Trend-Vol 5m zu klein ($${m.volume5mUsd.toFixed(1)})`);
    if (!avgVolumeEnough)
        redFlags.push(`Trend avg5m zu klein ($${m.avgVolume5mUsd.toFixed(1)})`);
    if (!volumeToLiquidityHealthy)
        redFlags.push(`Trend vol/liq schwach (${volToLiquidity.toFixed(2)})`);
    if (!sampleCountEnough)
        redFlags.push(`Trend Samples zu wenig (${m.sampleCount})`);
    if (!freshTrade)
        redFlags.push(`Trend-Flow alt (${secondsSinceTrade}s)`);
    if (!momentum5mGood)
        redFlags.push(`Trend 5m ungeeignet (${(m.change5mPct * 100).toFixed(2)}%)`);
    if (!momentum15mGood)
        redFlags.push(`Trend 15m ungeeignet (${(m.change15mPct * 100).toFixed(2)}%)`);
    if (!reboundHealthy)
        redFlags.push(`Trend Rebound ungeeignet (${(m.reboundFrom5mLowPct * 100).toFixed(2)}%)`);
    if (!notDumped)
        redFlags.push(`Trend zu weit unter 1h-High (${(m.dropFrom1hHighPct * 100).toFixed(2)}%)`);
    if (!notOverextended)
        redFlags.push("Trend bereits überdehnt");
    if (!notDead)
        redFlags.push("Trend Coin wirkt tot");
    const reasons = [];
    if (liquidityEnough)
        reasons.push(`Trend-Liq $${m.liquidityUsd.toFixed(1)}`);
    if (volumeEnough)
        reasons.push(`Trend-Vol $${m.volume5mUsd.toFixed(1)}`);
    if (avgVolumeEnough)
        reasons.push(`Trend-avg5m $${m.avgVolume5mUsd.toFixed(1)}`);
    if (momentum15mGood)
        reasons.push(`15m ${(m.change15mPct * 100).toFixed(2)}%`);
    if (momentum5mGood)
        reasons.push(`5m ${(m.change5mPct * 100).toFixed(2)}%`);
    if (freshTrade)
        reasons.push(`Trend-Flow ${secondsSinceTrade}s`);
    const buy = ENABLE_TREND_MODE &&
        oldEnough &&
        ageAllowed &&
        liquidityEnough &&
        volumeEnough &&
...
export function shouldBuySignal(m) {
    const now = Date.now();
...
export function shouldSellSignal(pos, m) {
    ...
    else if (momentumFailHit)
        reason = "momentum_fail";
    else if (stalePriceHit)
        reason = "stale_price";
    else if (maxHoldHit)
        reason = "max_hold";
    return {
