import {
    ...
    liveBuyPumpMint,
    liveSellPumpMint,
} from "./live/pumpExecution.js";
import {
    getAllMetrics,
    getMintMetrics,
    refreshWatchedMints,
    removeWatchedMint,
} from "./live/pumpStream.js";
...
type CandidateMemory = {
    ...
    lastAiScore: number;
    lastPrice: number;
    localHighPrice: number;
    lastMode: "SNIPER_MODE" | "TREND_MODE";
};
...
type LearningProfile = {
    ...
    universeReadyRate: number;
    universeAvgMovePct: number;
    universeAvgDrawdownPct: number;
    universePositiveCloseRate: number;
    universeFollowThroughRate: number;
    enabled: boolean;
    phase: 'DISABLED' | 'WARMUP' | 'TRAINING' | 'ADAPTIVE' | 'OPTIMIZED';
    sampleSize: number;
    totalClosedTrades: number;
    recentWinRate: number;
    recentAvgPnlPct: number;
    recentAvgWinPct: number;
    recentAvgLossPct: number;
    lifetimeWinRate: number;
    lifetimeAvgPnlPct: number;
    confidenceScore: number;
    consecutiveLosses: number;
    sizeMultiplier: number;
    minAiScoreBoost: number;
    avoidLateEntries: boolean;
    preferOnlyPreferredEntries: boolean;
    watchlistObserved: number;
    watchlistReadyRate: number;
    watchlistAvgScore: number;
    watchlistAvgAiScore: number;
    watchlistQualityScore: number;
    watchlistBias: number;
    watchlistAvgMovePct: number;
    watchlistAvgDrawdownPct: number;
    watchlistPositiveCloseRate: number;
    watchlistFollowThroughRate: number;
    watchlistFreshSamples: number;
    watchlistSummary: string;
    summary: string;
};

type EntryQualityDecision = {
    allowed: boolean;
    label: "EARLY" | "PREFERRED" | "LATE";
    summary: string;
    reasons: string[];
...
function assessEntryQuality(
    ...
    const pullbackFromLocalHighPct =
        memory.localHighPrice > 0 ? 1 - currentPrice / memory.localHighPrice : 0;
    const scoreVsBestPct = memory.bestScore > 0 ? score / memory.bestScore - 1 : 0;
    const matureSignal =
        stableSeconds >= ENTRY_CONFIRM_SECONDS && memory.seenCount >= ENTRY_CONFIRM_CYCLES;
    ...
    const pullbackHealthy =
        pullbackFromLocalHighPct >= ENTRY_PREFERRED_PULLBACK_PCT / 2 &&
    ...
    let allowed = false;
    let label: EntryQualityDecision["label"] = "EARLY";
    ...
    if (!matureSignal) reasons.push(`warte Bestätigung (${memory.seenCount}/${ENTRY_CONFIRM_CYCLES}, ${stableSeconds}s)`);
    if (!notChasing) reasons.push(`zu nah am Ausbruch (${fmtPct(Math.abs(pullbackFromLocalHighPct))})`);
    if (!pullbackHealthy) reasons.push(`kein sauberer Pullback (${fmtPct(pullbackFromLocalHighPct)})`);
    if (!notFallingKnife) reasons.push(`Pullback zu tief (${fmtPct(pullbackFromLocalHighPct)})`);
    if (!scoreStillStrong) reasons.push(`Score fällt ggü. Peak (${fmtPct(scoreVsBestPct)})`);
    if (!aiStrongEnough) reasons.push(`AI zu schwach (${aiScore.toFixed(1)})`);
...
function buildWatchlistLearningStats() {
    const activeRows = typeof getWatchlistLearningSnapshots === "function"
        ...
        : (getRuntimeWatchlist as any)(WATCHLIST_MAX_SIZE);
    ...
    const latestByMint = new Map<string, {
        score: number;
        aiScore: number;
        ready: boolean;
    ...
    for (const row of recentDecisions.reverse()) {
        ...
        const entry = (details.entry || {}) as any;
    ...
    const qualityScore = clamp(Number((
        readyRate * 30 +
        (avgScore / 100) * 16 +
        (avgAiScore / 100) * 14 +
...
function buildLearningProfile(): LearningProfile {
    if (!LEARNING_ENABLED) {
        return {
            universeObserved: 0,
            universeReadyRate: 0,
    ...
    for (const trade of closedTrades) {
        if (Number(trade.pnl_sol || 0) > 0) break;
        consecutiveLosses += 1;
    }
    ...
    if (closedTrades.length < LEARNING_MIN_CLOSED_TRADES) {
        ...
        const warmupSizeMultiplier = clamp(Number((
            1 +
            watchlistIntel.bias * 0.08 +
            (watchlistIntel.qualityScore >= 60 && watchlistIntel.observed >= 6 ? 0.03 : 0) -
            (watchlistIntel.qualityScore <= 28 && watchlistIntel.observed >= 6 ? 0.05 : 0)
        ).toFixed(2)), 0.8, 1.08);
        const warmupAiBoost = clamp(
            (watchlistIntel.qualityScore <= 28 && watchlistIntel.observed >= 6 ? 2 : 0) +
            (watchlistIntel.bias < -0.08 ? 1 : 0) +
            (watchlistIntel.bias > 0.12 ? -1 : 0),
            -1,
            3,
        );
    ...
    let phase: LearningProfile['phase'] = 'TRAINING';
    if (totalClosedTrades >= 100 && recentWinRate >= LEARNING_STRONG_WINRATE && recentAvgPnlPct > 0) phase = 'OPTIMIZED';
    else if (totalClosedTrades >= 30 || closedTrades.length >= Math.max(10, LEARNING_MIN_CLOSED_TRADES * 2)) phase = 'ADAPTIVE';

    const consistencyBoost = recentAvgPnlPct >= 0 ? 8 : -8;
    const confidenceScore = clamp(Number((
        30 +
...
function isEntryAllowedByLearning(entry: EntryQualityDecision, learning: LearningProfile) {
    if (!learning.enabled || learning.sampleSize < LEARNING_MIN_CLOSED_TRADES) return true;
    if (learning.preferOnlyPreferredEntries && entry.label !== 'PREFERRED') return false;
    if (learning.avoidLateEntries && entry.label === 'LATE') return false;
    return true;
}

function classifyWatchStatus(signal: ReturnType<typeof shouldBuySignal>, entry: EntryQualityDecision, memory: CandidateMemory) {
    const stableSeconds = Number(entry.data.stableSeconds || 0);
    const scoreFloor = Math.max(WATCHLIST_MIN_SCORE, ENTRY_AI_MIN_SCORE - 6);
    const keepWatching =
        signal.score >= scoreFloor ||
    ...
    if (entry.allowed && signal.score >= WATCHLIST_READY_MIN_SCORE && signal.ai.aiScore >= ENTRY_AI_MIN_SCORE) {
        return { keep: true, status: "READY" as const, summary: `${entry.label} • Watchlist-Kauf freigegeben` };
    }
    ...
    if (signal.score <= WATCHLIST_REJECT_DROP_SCORE && stableSeconds < ENTRY_CONFIRM_SECONDS) {
        return { keep: false, status: "COOLDOWN" as const, summary: "zu schwach für Watchlist" };
    }

    return { keep: false, status: "COOLDOWN" as const, summary: "Signal verloren" };
}

function persistCandidateState(item: { metrics: any; signal: ReturnType<typeof shouldBuySignal>; memory: CandidateMemory; entry: EntryQualityDecision; }) {
...
function syncPosition(position: Position) {
    upsertRuntimePosition({
        mint: position.mint,
...
function candidatePriority(item: { signal: ReturnType<typeof shouldBuySignal>; memory: CandidateMemory; entry: EntryQualityDecision; }) {
    const seenBoost = Math.min(12, item.memory.seenCount * 1.5);
    const stableBoost = Math.min(8, Math.max(0, item.entry.data.stableSeconds as number || 0) / 6);
...
function getLossStreakState() {
    ...
    for (const row of decisions) {
        if (row.action === "sell_win") break;
        if (row.action === "sell_loss") {
            consecutiveLosses += 1;
            if (!lastLossAt) lastLossAt = new Date(row.created_at).getTime();
        }
    }
...
async function trySell() {
    ...
    for (let i = activePositions.length - 1; i >= 0; i--) {
        ...
        console.log(
            `📦 HOLD ${pos.symbol} | pnl=${fmtPct(decision.pnlPct)} | high=$${fmtNum(
                pos.highestPriceSeen
            )} | drawdown=${fmtPct(decision.trailingDrawdown)} | 5m=${fmtPct(
                metrics.change5mPct
            )} | 15m=${fmtPct(metrics.change15mPct)} | 1h=${fmtPct(metrics.change1hPct)}`
        );
...
function computePositionSize(score: number, sizeMultiplier: number, learning: LearningProfile) {
    const controls = getDashboardControlState();
...
async function tryBuy() {
    ...
    const candidates = getAllMetrics()
        .filter((m) => {
            ...
            if (secondsSinceTrade > STALE_TRADE_SECONDS && !existingWatch) return false;
            return true;
        })
        .map((metrics) => {
    ...
    for (const { metrics: c, signal, entry, watch } of candidates.slice(0, 8)) {
        console.log(
            `🎯 ${c.symbol} | mode=${signal.mode.replace("_MODE", "")} | score=${signal.score.toFixed(1)} | ai=${signal.ai.aiScore.toFixed(1)} ${signal.ai.decision} | price=${fmtNum(c.currentPrice)} | liq=$${c.liquidityUsd.toFixed(
                2
            )} | vol5m=$${c.volume5mUsd.toFixed(2)} | 5m=${fmtPct(c.change5mPct)} | 15m=${fmtPct(c.change15mPct)} | watch=${watch?.status || "DROP"} | entry=${entry.label} ${entry.allowed ? "OK" : "WAIT"}`
        );
    }

    for (const { metrics: m, signal, entry, watch } of readyCandidates) {
        const persisted = getWatchCandidate(m.mint);
        if (!persisted?.readyToBuy) continue;
        ...
        try {
            console.log(
                `🔥 WATCHLIST BUY ${m.symbol} | mode=${signal.mode.replace("_MODE", "")} | score=${signal.score.toFixed(1)} | ai=${signal.ai.aiScore.toFixed(1)} ${signal.ai.decision} | entry=${entry.label} | size=${finalSizeSol.toFixed(4)} SOL | entryPrice=${fmtNum(
                    m.currentPrice
                )}`
            );
