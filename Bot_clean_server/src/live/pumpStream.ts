...
export type MintMetrics = {
    ...
    dropFrom1hHighPct: number;
    reboundFrom5mLowPct: number;

    avgVolume5mUsd: number;
...
function getWindow(points: PricePoint[], ms: number) {
    const cutoff = now() - ms;
    return points.filter((p) => p && isValidNumber(p.price) && p.ts >= cutoff);
}

function getVolumeWindow(points: VolumePoint[], ms: number) {
    const cutoff = now() - ms;
    return points.filter((p) => p && isValidNumber(p.vol) && p.ts >= cutoff);
}

function pctChange(from: number, to: number) {
    if (!isValidNumber(from) || !isValidNumber(to) || from <= 0) return 0;
    return to / from - 1;
}

function getHigh(points: PricePoint[]) {
    if (!points.length) return 0;
    return Math.max(...points.map((p) => p.price));
}

function getLow(points: PricePoint[]) {
    if (!points.length) return 0;
    return Math.min(...points.map((p) => p.price));
}

function cleanupOldData(state: MintState) {
...
export function upsertWatchedMint(mint: string, symbol: string) {
...
export function removeWatchedMint(mint: string) {
    mints.delete(mint);
}
...
export function ingestPriceUpdate(data: {
    ...
    if (!isValidNumber(data.price) || data.price <= 0) return;
...
export function getMintMetrics(mint: string): MintMetrics | null {
    const s = mints.get(mint);
    if (!s) return null;
    if (!Array.isArray(s.prices) || s.prices.length < 1) return null;

    const validPrices = s.prices.filter((p) => p && isValidNumber(p.price) && p.price > 0);
    if (validPrices.length < 1) return null;
    ...
    const dropFrom1hHighPct =
        high1h > 0 ? currentPrice / high1h - 1 : 0;

    const reboundFrom5mLowPct =
        low5m > 0 ? currentPrice / low5m - 1 : 0;

    const vol5mPoints = getVolumeWindow(s.volumeHistory || [], 5 * 60 * 1000);

    const volume5mUsd =
        vol5mPoints.length > 0
            ? vol5mPoints[vol5mPoints.length - 1]?.vol ?? 0
            : 0;

    const avgVolume5mUsd =
        vol5mPoints.length > 0
...
export function getAllMetrics(): MintMetrics[] {
    const result: MintMetrics[] = [];
...
export function getMintPriceSeries(mint: string, limit = 32) {
    const s = mints.get(mint);
    if (!s || !Array.isArray(s.prices) || s.prices.length < 1) return [];
