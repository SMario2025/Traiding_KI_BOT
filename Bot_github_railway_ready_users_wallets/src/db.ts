...
type RuntimeWatchlistStatus = "WATCH" | "READY" | "COOLDOWN";

type RuntimeWatchlistRow = {
  mint: string;
  symbol: string;
  status: RuntimeWatchlistStatus;
  score: number;
  aiScore: number;
  bestScore: number;
  seenCount: number;
...
type EquitySnapshotRow = {
  ...
  ts: number;
  wallet_sol: number;
  open_positions_value_sol: number;
  realized_pnl_sol: number;
  total_equity_sol: number;
  open_positions: number;
  watched_mints: number;
};
...
type SeenMintRow = {
  ...
  last_volume5m_usd: number;
  last_volume24h_usd: number;
  best_score: number;
  last_score: number;
  best_ai_score: number;
  last_ai_score: number;
  ready_hits: number;
  watch_hits: number;
  positive_ticks: number;
  source: string;
  entry_label: string;
  updated_at: number;
};
...
type LearningHistoryRow = {
  ...
  min_ai_score_boost: number;
  recent_win_rate: number;
  recent_avg_pnl_pct: number;
  total_closed_trades: number;
  watchlist_quality_score: number;
  watchlist_observed: number;
  summary: string;
};
...
type Store = {
  ...
  price_history: Array<{ id: number; pair: string; price: number; created_at: string }>;
  bot_trade_log: BotTradeRow[];
  runtime_positions: RuntimePositionRow[];
  runtime_watchlist: RuntimeWatchlistRow[];
  equity_snapshots: EquitySnapshotRow[];
  bot_events: BotEventRow[];
  bot_decisions: BotDecisionRow[];
  ai_snapshots: AiSnapshotRow[];
  mint_creators: MintCreatorRow[];
...
function normalizeStore(parsed: Partial<Store>): Store {
  ...
  return {
    ...
    user_sessions: (parsed as any).user_sessions || [],
  };
}

function tryReadStore(filePath: string): Store | null {
...
function loadStore(fallback: Store = store): Store {
...
function replaceFileRobust(tempPath: string, targetPath: string) {
...
function limitLatest<T>(arr: T[], max: number) {
  if (arr.length > max) arr.splice(0, arr.length - max);
}

export const db = { kind: 'json-store', path: jsonPath };

export function insertTrade(params: { pair: string; side: 'BUY' | 'SELL'; price: number; amountSol: number; amountOut: number; pnl?: number; status?: string; notes?: string; tx?: string; }) {
...
export function getTrades(limit = 100) {
  refreshStore(); return [...store.trades].sort((a,b)=>b.id-a.id).slice(0, limit); }
export function getTradesByPair(pair: string, limit = 100) {
  refreshStore(); return getTrades(10000).filter(x=>x.pair===pair).slice(0, limit); }

export function setOpenPosition(params: { pair: string; entryPrice: number; amountSol: number; amountToken: number; }) {
  const existing = store.positions.find(x=>x.pair===params.pair);
  if (existing) {
    existing.entry_price = params.entryPrice; existing.amount_sol = params.amountSol; existing.amount_token = params.amountToken; existing.opened_at = nowIso();
  } else {
...
export function getOpenPosition(pair: string) {
  refreshStore(); return store.positions.find(x=>x.pair===pair); }
export function getAllOpenPositions() {
  refreshStore(); return [...store.positions].sort((a,b)=>b.id-a.id); }
export function closePosition(pair: string) {
  refreshStore(); store.positions = store.positions.filter(x=>x.pair!==pair); saveStore(); }

export function insertPrice(pair: string, price: number) {
  refreshStore(); store.price_history.push({ id: nextId('price_history'), pair, price, created_at: nowIso() }); limitLatest(store.price_history, 10000); saveStore(); }
export function getRecentPrices(pair: string, limit = 100) {
  refreshStore(); return store.price_history.filter(x=>x.pair===pair).sort((a,b)=>b.id-a.id).slice(0, limit).map(x=>Number(x.price)).reverse(); }
export function trimPriceHistory(keepPerPair = 500) {
  refreshStore();
  const grouped = new Map<string, typeof store.price_history>();
...
export function getLastBuyTime(pair: string): number | null {
  refreshStore(); const row = getTradesByPair(pair, 1000).find(x=>x.side==='BUY'); return row ? new Date(row.created_at).getTime() : null; }
export function getTotalRealizedPnl() {
  refreshStore(); return Number(store.trades.filter(x=>x.side==='SELL').reduce((s,x)=>s+Number(x.pnl||0), 0)); }
...
export function logBotTrade(params: { mint: string; symbol: string; side: 'BUY'|'SELL'; entryPrice?: number; exitPrice?: number; sizeSol: number; pnlPct?: number; pnlSol?: number; reason?: string; route?: string; txid?: string; status?: string; }) {
  refreshStore();
...
export function getBotTrades(limit = 100) {
  refreshStore(); return [...store.bot_trade_log].sort((a,b)=>b.id-a.id).slice(0, limit); }
...
export function upsertRuntimePosition(params: { mint: string; symbol: string; entryTime: number; entryPrice: number; highestPriceSeen: number; sizeSol: number; buyTxid: string; route: string; }) {
  refreshStore();
  const existing = store.runtime_positions.find(x=>x.mint===params.mint);
  const row: RuntimePositionRow = { mint: params.mint, symbol: params.symbol, entry_time: params.entryTime, entry_price: params.entryPrice, highest_price_seen: params.highestPriceSeen, size_sol: params.sizeSol, buy_txid: params.buyTxid, route: params.route, updated_at: Date.now() };
  if (existing) Object.assign(existing, row); else store.runtime_positions.push(row);
  saveStore();
}
export function removeRuntimePosition(mint: string) {
  refreshStore(); store.runtime_positions = store.runtime_positions.filter(x=>x.mint!==mint); saveStore(); }
export function getRuntimePositions() {
  refreshStore(); return [...store.runtime_positions].sort((a,b)=>b.entry_time-a.entry_time); }

export function insertEquitySnapshot(params: { ts?: number; walletSol: number; openPositionsValueSol: number; realizedPnlSol: number; totalEquitySol: number; openPositions: number; watchedMints: number; }) {
  refreshStore();
...
export function getRecentEquitySnapshots(limit = 200) {
  refreshStore(); return [...store.equity_snapshots].sort((a,b)=>b.id-a.id).slice(0, limit).reverse(); }

export function insertBotEvent(level: 'info'|'warn'|'error'|'success', type: string, message: string, meta?: unknown) {
  refreshStore();
  store.bot_events.push({ id: nextId('bot_events'), level, type, message, meta: (meta as Record<string, unknown>) || {}, created_at: nowIso() });
  limitLatest(store.bot_events, 5000);
  saveStore();
}

export function logBotDecision(params: { mint: string; symbol: string; action: string; score?: number; summary?: string; details?: unknown; }) {
  refreshStore();
  store.bot_decisions.push({ id: nextId('bot_decisions'), mint: params.mint, symbol: params.symbol, action: params.action, score: params.score ?? 0, summary: params.summary ?? '', details: (params.details as Record<string, unknown>) || {}, created_at: nowIso() });
  limitLatest(store.bot_decisions, 8000);
  saveStore();
}
export function getRecentBotDecisions(limit = 100) {
  refreshStore(); return [...store.bot_decisions].sort((a,b)=>b.id-a.id).slice(0, limit); }
export function getActiveWatchlist(limit = 12) {
  refreshStore(); return getRuntimeWatchlist(limit).filter(x=>x.status!=='COOLDOWN'); }
export function getDecisionSummary(limit = 500) {
  refreshStore();
  const counts = new Map<string, number>();
  for (const row of getRecentBotDecisions(limit)) counts.set(row.action, (counts.get(row.action) || 0) + 1);
  return [...counts.entries()].map(([action, count])=>({ action, count })).sort((a,b)=>b.count-a.count);
}
export function getRecentBotEvents(limit = 100) {
  refreshStore(); return [...store.bot_events].sort((a,b)=>b.id-a.id).slice(0, limit); }

export function setBotHealth(params: { status?: string; lastLoopAt?: number; lastFeedAt?: number; lastBuyAt?: number; lastSellAt?: number; details?: Record<string, unknown>; }) {
  refreshStore();
  if (params.status !== undefined) store.bot_health.status = params.status;
  if (params.lastLoopAt !== undefined) store.bot_health.lastLoopAt = params.lastLoopAt;
  if (params.lastFeedAt !== undefined) store.bot_health.lastFeedAt = params.lastFeedAt;
  if (params.lastBuyAt !== undefined) store.bot_health.lastBuyAt = params.lastBuyAt;
  if (params.lastSellAt !== undefined) store.bot_health.lastSellAt = params.lastSellAt;
...
export function getBotHealth() {
  refreshStore(); return { ...store.bot_health }; }

export function upsertMintCreator(params: { mint: string; symbol: string; creatorWallet?: string | null }) {
  refreshStore();
  if (!params.creatorWallet) return;
...
export function getMintCreator(mint: string) {
  refreshStore(); return store.mint_creators.find(x=>x.mint===mint); }
export function getWalletProfile(wallet: string) {
  refreshStore(); return store.wallet_profiles.find(x=>x.wallet===wallet); }
...
export function recordMintOutcome(params: { mint: string; pnlPct: number; pnlSol: number }) {
  refreshStore();
  const creator = getMintCreator(params.mint);
  if (!creator || creator.outcome_recorded) return;
  ...
  if (profile) {
    ...
    if (params.pnlSol > 0) profile.wins += 1; else profile.losses += 1;
...
export function getTopWalletProfiles(limit = 10) {
  refreshStore(); return [...store.wallet_profiles].sort((a,b)=>(b.confidence_score||0)-(a.confidence_score||0)).slice(0, limit); }

export function insertAiSnapshot(params: { aiEnabled: boolean; marketRegime: string; aiScoreAvg: number; aiScoreBest: number; allows: number; rejects: number; reduceSize: number; highConfidence: number; }) {
  refreshStore();
...
export function getLatestAiSnapshot() {
  refreshStore(); return [...store.ai_snapshots].sort((a,b)=>b.id-a.id)[0] || null; }

export function upsertRuntimeWatchlist(params: { mint: string; symbol: string; status: RuntimeWatchlistStatus; score: number; aiScore: number; bestScore: number; seenCount: number; stableSeconds: number; pullbackFromLocalHighPct: number; currentPrice: number; change5mPct: number; change15mPct: number; change1hPct: number; summary: string; reasons: string[]; readyToBuy: boolean; entryLabel: string; }) {
  refreshStore();
  const existing = store.runtime_watchlist.find(x=>x.mint===params.mint);
  const row: RuntimeWatchlistRow = { mint: params.mint, symbol: params.symbol, status: params.status, score: params.score, aiScore: params.aiScore, bestScore: params.bestScore, seenCount: params.seenCount, stableSeconds: params.stableSeconds, pullbackFromLocalHighPct: params.pullbackFromLocalHighPct, currentPrice: params.currentPrice, change5mPct: params.change5mPct, change15mPct: params.change15mPct, change1hPct: params.change1hPct, summary: params.summary, reasons: params.reasons, readyToBuy: params.readyToBuy, entryLabel: params.entryLabel, updatedAt: Date.now(), created_at: existing?.created_at || nowIso() };
  if (existing) Object.assign(existing, row); else store.runtime_watchlist.push(row);
  limitLatest(store.runtime_watchlist, 1000);
  saveStore();
}
export function deleteRuntimeWatchlist(mint: string) {
  refreshStore(); store.runtime_watchlist = store.runtime_watchlist.filter(x=>x.mint!==mint); saveStore(); }
export function getRuntimeWatchlistByMint(mint: string) {
  refreshStore(); return store.runtime_watchlist.find(x=>x.mint===mint); }
...
export function upsertSeenMintObservation(params: {
  ...
  const row: SeenMintRow = existing || {
    mint: params.mint,
  ...
  if (price > 0) {
    if (row.first_price <= 0) row.first_price = price;
    if (row.last_price > 0 && price > row.last_price) row.positive_ticks += 1;
  ...
  if (params.status === 'READY') row.ready_hits += 1;
  if (params.status === 'WATCH' || params.status === 'READY') row.watch_hits += 1;
  if (!existing) store.seen_mints.push(row);
...
export function insertLearningSnapshot(params: { ts?: number; phase: string; confidenceScore: number; sizeMultiplier: number; minAiScoreBoost: number; recentWinRate: number; recentAvgPnlPct: number; totalClosedTrades: number; watchlistQualityScore: number; watchlistObserved: number; summary: string; }) {
  refreshStore();
...
export function getUsers() {
  refreshStore();
  return [...store.users].sort((a,b)=>b.id-a.id).map((x)=>({
    id: x.id,
    username: x.username,
    wallet_public_key: x.wallet_public_key,
    balance_sol: Number(x.balance_sol || 0),
    bot_enabled: Boolean(x.bot_enabled),
    created_at: x.created_at,
    updated_at: x.updated_at,
    last_login_at: Number(x.last_login_at || 0),
  }));
}

export function getUserByUsername(username: string) {
  refreshStore();
  return store.users.find((x)=>x.username.toLowerCase()===String(username||'').trim().toLowerCase()) || null;
}

export function getUserById(userId: number) {
  refreshStore();
  return store.users.find((x)=>Number(x.id)===Number(userId)) || null;
}
...
export function createUser(params: {
  ...
  refreshStore();
  const existing = getUserByUsername(params.username);
  if (existing) throw new Error('username_exists');
  const row: UserRow = {
    id: nextId('users'),
    username: String(params.username || '').trim(),
    password_hash: String(params.passwordHash || ''),
    wallet_public_key: String(params.walletPublicKey || ''),
    wallet_secret_key: String(params.walletSecretKey || ''),
    balance_sol: 0,
    bot_enabled: false,
    created_at: nowIso(),
    updated_at: nowIso(),
...
export function setUserBalanceSol(userId: number, balanceSol: number) {
  refreshStore();
  const user = store.users.find((x)=>Number(x.id)===Number(userId));
  if (!user) return null;
  user.balance_sol = Number(balanceSol || 0);
  user.updated_at = nowIso();
  saveStore();
  return getUserById(userId);
}

export function setUserBotEnabled(userId: number, enabled: boolean) {
  refreshStore();
  const user = store.users.find((x)=>Number(x.id)===Number(userId));
  if (!user) return null;
  user.bot_enabled = Boolean(enabled);
  user.updated_at = nowIso();
  saveStore();
  return getUserById(userId);
}

export function touchUserLogin(userId: number) {
  refreshStore();
  const user = store.users.find((x)=>Number(x.id)===Number(userId));
  if (!user) return null;
...
export function createUserSession(userId: number, token: string, expiresAt: number) {
  refreshStore();
  store.user_sessions = store.user_sessions.filter((x)=>x.token!==token && x.expires_at > Date.now());
  store.user_sessions.push({
    token,
    user_id: Number(userId),
    created_at: nowIso(),
    expires_at: Number(expiresAt || 0),
  });
  limitLatest(store.user_sessions, 5000);
  saveStore();
}

export function getUserBySessionToken(token: string) {
  refreshStore();
  const session = store.user_sessions.find((x)=>x.token===token && Number(x.expires_at || 0) > Date.now());
...
export function deleteUserSession(token: string) {
  refreshStore();
  store.user_sessions = store.user_sessions.filter((x)=>x.token!==token);
  saveStore();
}

export function getUserSystemSummary() {
  refreshStore();
  const users = getUsers();
  const enabled = users.filter((x)=>x.bot_enabled).length;
  const totalBalanceSol = users.reduce((s,x)=>s+Number(x.balance_sol || 0), 0);
  return {
    totalUsers: users.length,
    enabledBots: enabled,
    disabledBots: users.length - enabled,
    totalBalanceSol: Number(totalBalanceSol.toFixed(4)),
  };
}


export function getDashboardControlState() {
  refreshStore(); return { ...store.dashboard_controls, abortMints: [...store.dashboard_controls.abortMints] }; }
export function updateDashboardControlState(patch: Partial<DashboardControlState>) {
  refreshStore();
  if (patch.killSwitch !== undefined) store.dashboard_controls.killSwitch = Boolean(patch.killSwitch);
  if (patch.sizeMultiplier !== undefined) store.dashboard_controls.sizeMultiplier = Math.max(0.25, Math.min(2, Number(patch.sizeMultiplier)));
  if (patch.maxPositionsOverride !== undefined) store.dashboard_controls.maxPositionsOverride = patch.maxPositionsOverride == null ? null : Math.max(1, Math.min(10, Number(patch.maxPositionsOverride)));
  if (patch.abortAll !== undefined) store.dashboard_controls.abortAll = Boolean(patch.abortAll);
  if (patch.abortMints !== undefined) store.dashboard_controls.abortMints = Array.from(new Set((patch.abortMints || []).map(String)));
  store.dashboard_controls.updatedAt = Date.now();
  saveStore();
  return getDashboardControlState();
}
export function requestAbortMint(mint: string) {
  refreshStore();
  if (!store.dashboard_controls.abortMints.includes(mint)) store.dashboard_controls.abortMints.push(mint);
  store.dashboard_controls.updatedAt = Date.now();
  saveStore();
}
export function clearAbortMint(mint: string) {
  refreshStore();
