...
import { deleteRuntimeWatchlist, getRuntimeWatchlist, getRuntimeWatchlistByMint, upsertRuntimeWatchlist, } from "./db.js";
...
export function removeWatchCandidate(mint) {
    deleteRuntimeWatchlist(mint);
}
export function getWatchCandidate(mint) {
    return getRuntimeWatchlistByMint(mint);
}
function computeMovementStats(mint) {
...
export function getWatchlistSnapshots(limit = 24) {
    return getRuntimeWatchlist(limit).map((item) => ({
        ...item,
