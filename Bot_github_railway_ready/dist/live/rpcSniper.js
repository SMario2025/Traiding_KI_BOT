...
function isFreshPumpMint(mint) {
    return typeof mint === "string" && mint.endsWith("pump") && mint.length > 20;
}
function pushCandidate(mint, source) {
    const cleanMint = String(mint || "").trim();
    if (!isFreshPumpMint(cleanMint))
        return;
    if (seenMints.has(cleanMint))
        return;
...
async function tryExtractMintFromTx(signature) {
    const now = Date.now();
...
export async function startRpcSniper() {
    ...
    if (PUMP_PROGRAM_ID) {
        ...
        subscriptionId = connection.onLogs(new PublicKey(PUMP_PROGRAM_ID), async (logInfo) => {
    ...
    subscriptionId = connection.onLogs("all", async (logInfo) => {
        const signature = logInfo.signature;
        if (!signature || seenSignatures.has(signature))
            return;
...
export async function getRpcSniperCandidates() {
    if (!started) {
