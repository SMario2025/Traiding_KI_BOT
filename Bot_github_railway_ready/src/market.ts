...
export async function buyToken(pair: string, solAmount: number) {
    const { baseMint, quoteMint } = getMints(pair)
    ...
    if (!json?.data) throw new Error("Buy fehlgeschlagen")
...
export async function sellToken(pair: string, tokenAmount: number) {
    const { baseMint, quoteMint } = getMints(pair)
    ...
    if (!json?.data) throw new Error("Sell fehlgeschlagen")
