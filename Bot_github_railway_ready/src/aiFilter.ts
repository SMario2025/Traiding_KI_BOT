...
export type AiDecision = {
  ...
  marketRegime: "DEAD" | "NORMAL" | "HYPE";
  decision: "ALLOW" | "REJECT" | "REDUCE_SIZE" | "HIGH_CONFIDENCE";
...
function getSessionBoost(hourBerlin: number) {
  if (hourBerlin >= 18 || hourBerlin < 2) return 9;
  if (hourBerlin >= 12 && hourBerlin < 18) return 4;
  if (hourBerlin >= 9 && hourBerlin < 12) return -3;
  return -7;
}

function detectMarketRegime(m: MintMetrics, secondsSinceTrade: number) {
  if (
    m.liquidityUsd < AI_DEAD_MARKET_LIQUIDITY ||
...
export function runAiFilter(params: {
  ...
  if (marketRegime === "DEAD") aiScore -= 10;
  if (walletProfile) {
    ...
    if (closedLaunches >= 3) aiScore += 4;
  }
  if (marketRegime === "HYPE") aiScore += 5;
  if (mode === "TREND_MODE") aiScore += 3;
  if (ageSeconds <= 45) aiScore -= 4;
  if (secondsSinceTrade <= 2) aiScore += 3;
  if (secondsSinceTrade > 6) aiScore -= 6;
  if (m.change5mPct > 0.28) aiScore -= 7;
  if (m.reboundFrom5mLowPct > 0.16) aiScore -= 5;
  if (m.dropFrom1hHighPct < -0.18) aiScore -= 8;
  ...
  if (m.liquidityUsd >= 80) reasons.push(`AI mag Liq $${m.liquidityUsd.toFixed(1)}`);
  if (walletProfile && Number(walletProfile.confidence_score || 0) >= 65) reasons.push(`AI kennt Wallet (${Number(walletProfile.confidence_score || 0).toFixed(0)})`);
  if (m.volume5mUsd >= 35) reasons.push(`AI mag Flow $${m.volume5mUsd.toFixed(1)}`);
  if (volToLiquidity >= 0.5) reasons.push(`AI mag vol/liq ${volToLiquidity.toFixed(2)}`);
  if (secondsSinceTrade <= 3) reasons.push(`AI sieht frischen Tape ${secondsSinceTrade}s`);
  if (marketRegime === "HYPE") reasons.push("AI erkennt Hype-Markt");
  if (hourBerlin >= 18 || hourBerlin < 2) reasons.push("AI mag US Session");

  if (marketRegime === "DEAD") redFlags.push("AI erkennt dead market");
  if (m.sampleCount < 4) redFlags.push(`AI: Samples dünn (${m.sampleCount})`);
  if (secondsSinceTrade > 6) redFlags.push(`AI: Tape alt (${secondsSinceTrade}s)`);
  if (m.change5mPct > 0.28) redFlags.push(`AI: Spike überhitzt (${(m.change5mPct * 100).toFixed(1)}%)`);
  if (m.reboundFrom5mLowPct > 0.16) redFlags.push(`AI: Rebound zu steil (${(m.reboundFrom5mLowPct * 100).toFixed(1)}%)`);
  if (hourBerlin < 10) redFlags.push("AI: schwache Morgenphase Berlin");
  if (walletProfile && Number(walletProfile.confidence_score || 0) < 45 && Number(walletProfile.closed_launches || 0) >= 3) redFlags.push("AI: Wallet-Historie schwach");

  let decision: AiDecision["decision"] = "ALLOW";
  ...
  if (walletProfile) {
    ...
    if (confidence >= 75 && decision !== "REJECT") sizeMultiplier *= 1.18;
    else if (confidence <= 42 && decision !== "REJECT") sizeMultiplier *= 0.82;
  }
  sizeMultiplier = clamp(sizeMultiplier, 0, 1.35);

  const confidence: AiDecision["confidence"] =
    aiScore >= 78 ? "HIGH" : aiScore >= 62 ? "MEDIUM" : "LOW";
